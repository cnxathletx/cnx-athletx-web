...
async function logEmail(
  ...
  try {
    ...
    // Best effort — don't let logging failures propagate
  }
}
...
function itemsTableHtml(items: EmailItem[]): string {
  const rows = items
    .map(
      (item) =>
        `<tr>
          <td style="padding: 8px 0; border-bottom: 1px solid #E8E2D8; font-size: 14px;">${item.name}</td>
...
function buildOrderCreatedEmail(order: OrderEmailData, payment: PaymentInstructions): string {
  ...
  if (payment.promptpay_number) {
    paymentHtml += `<p style="margin: 8px 0 4px; font-size: 14px;"><strong>PromptPay:</strong> ${payment.promptpay_number}</p>`
  }

  if (payment.bank_name) {
    paymentHtml += `<p style="margin: 8px 0 4px; font-size: 14px;"><strong>Bank:</strong> ${payment.bank_name}</p>
    <p style="margin: 0 0 4px; font-size: 14px;"><strong>Account Name:</strong> ${payment.bank_account_name}</p>
    <p style="margin: 0 0 4px; font-size: 14px;"><strong>Account Number:</strong> ${payment.bank_account_number}</p>`
  }
...
async function fetchOrderEmailData(env: Env, orderId: string): Promise<OrderEmailData | null> {
...
router.post('/api/checkout', async (request: Request, env: Env) => {
  ...
  }).catch(() => {}) // Never block checkout response
...
router.post('/api/admin/orders/:id/ship', async (request: Request, env: Env) => {
  ...
  try {
    ...
    // Send Order Shipped email (fire-and-forget)
    fetchOrderEmailData(env, orderId).then((emailData) => {
