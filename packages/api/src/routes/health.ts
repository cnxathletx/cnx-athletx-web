...
export function registerHealthRoutes(router: RouterType) {
  ...
  router.post('/api/__test-loyalty-ledger', async (request: Request, env: Env) => {
    ...
    const pointsDelta = typeof body.points_delta === 'number' ? body.points_delta : Number.NaN
    const kind = typeof body.kind === 'string' ? body.kind : 'manual_adjustment'
    const reason = typeof body.reason === 'string' ? body.reason : 'test adjustment'
